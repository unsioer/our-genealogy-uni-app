<template>
    <table v-if="treeData.name">
      <tr>
        ...
        >
          <div :class="{node: true, hasmates: treeData.mates}">
            <div class="person" 
              :class="Array.isArray(treeData.class) ? treeData.class : []"
              @click="$emit('click-node', treeData)"
            >
              <div class="avat">
                <image :src="treeData.image_url" />
              </div>
              <div class="name">{{treeData.name}}</div>
            </div>
            <template v-if="Array.isArray(treeData.mates) && treeData.mates.length">
              <div class="person" v-for="(mates, matesIndex) in treeData.mates" :key="treeData.name+matesIndex"
                :class="Array.isArray(mates.class) ? mates.class : []"
                @click="$emit('click-node', mates)"
              >
                <div class="avat">
                  <img :src="mates.image_url" />
                </div>
